import { calcNullifier, DarkSwapError, DarkSwapMessage, DarkSwapNote, DarkSwapOrderNote } from '@thesingularitynetwork/darkswap-sdk';
...
export class SettlementService {
  ...
  async aliceSwap(orderInfo: OrderDto) {
    ...
    const darkSwapContext = await DarkSwapContext.createDarkSwapContext(orderInfo.chainId, orderInfo.wallet, this.rpcManager);
    //check note status
    ...
    if (aliceNoteOnChainStatus != NoteOnChainStatus.ACTIVE) {
      ...
      throw new DarkSwapError(`Order Note ${orderNote.note} is not active and no settlement transaction found`);
    }
